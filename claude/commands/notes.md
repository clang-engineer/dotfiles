...
## 작성 후: blog 승격 제안 (조건부)
- 방금 캡처한 내용 중 **외부 독자가 검색으로 들어와 가치를 얻을 만한 부분**이 있으면,
  notes 작성 완료 후 한 줄로 물어본다:
  "이 중 〈해당 주제〉는 공개 가치가 있어 보입니다. `/blog`로도 정리할까요?"
- 순수 개인용(명령어 모음·프로젝트 분석·나만의 참조)이면 **묻지 않는다.**
- 사용자가 수락하면 그 주제로 /blog 흐름을 이어간다.
