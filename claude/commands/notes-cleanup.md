...
3. **POST** — 이슈→원인→해결 형식의 트러블슈팅 **이면서 공개 가치를 통과한 것**
   ...
   - 게이트 **탈락** 시: 미래의 나에게 참고될 값이면 가까운 cheatsheet/doc에 흡수, 아니면 DELETE.
...
```
| 파일명 | 분류 | 공개가치 | 대상 경로/파일 | 한 줄 요약 |
```

판단 기준:
- 명령어/단축키 위주 → CHEATSHEET
- 설계/원리/구조 설명 → DOC
- "어떤 상황에서 안 됐고 어떻게 해결했다" → POST **후보**. 공개 가치 게이트 통과해야 POST 확정, 탈락하면 cheatsheet/doc 흡수 또는 DELETE
- 이미 cheatsheet/doc/blog에 있는 내용 → DELETE

분류표에 **공개가치** 열을 둔다 (POST 후보에 통과/탈락/턱걸이 표시).
...
- **공개 가치 게이트를 턱걸이로 통과/탈락한 POST 후보**를 따로 짚어서 최종 확인받기 (블로그行 여부는 애매하면 사람이 결정)
- 중복 병합 vs 별도 유지 결정
- 진행 방식 (일괄 vs 단계별)
