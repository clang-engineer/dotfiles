...
init.lua                              ← [Neovim] 시작점
 ...
 ├─ require("toolbox").setup()        ← 직접 require
 └─ require("quicklinks").setup()     ← 직접 require
```
...
| `db/` (init + connections) | `plugins/dadbod.lua`의 `init`이 `require("db")` |
| `toolbox/`, `quicklinks/` | `init.lua`가 `require(...).setup()` |

`db/connections/`에 파일 추가하면 자동 포함되는 건 LazyVim 폴더 스캔이 아니라 **`db/init.lua`가 직접 runtime glob을 돌리기 때문**이다.
