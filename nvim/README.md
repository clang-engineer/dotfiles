...
JDK 환경변수 주입은 `jvm-env` 모듈이 담당합니다. 상세 가이드는 [`lazy/lua/jvm-env/README.md`](lazy/lua/jvm-env/README.md) 참조.
...
dotfiles/
├── nvim/lazy/
│   ├── init.lua                      # 시작 시 jvm-env.setup() 호출
│   └── lua/jvm-env/                  # OS별 JDK 경로 탐지 + 환경변수 주입 모듈
└── nvim/opt/
...
1. nvim 시작 시 `init.lua`에서 `require("jvm-env").setup()` 호출
...
- `nvim/lazy/lua/jvm-env/` — JDK 탐지/환경변수 주입 모듈 (자체 README 포함)
