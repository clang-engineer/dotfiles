...
- **스크립트 활용**: 실제 키 생성과 config 추가는 `./scripts/unix/opt/generate-ssh-key.sh`, `./scripts/unix/opt/add-ssh-config.sh`, `./scripts/unix/setup-github-account.sh` 또는 Windows 대응 스크립트를 사용해 수동 실수를 줄입니다.
...
- 템플릿(`home/.secrets.example`, `home/.secrets.ps1.example`)이 저장소에 포함되어 있으니, 새 환경에서는 복사 후 값만 채우면 됩니다:
