...
- `hammerspoon/`
- 기타 비밀 또는 머신별 경로를 참조하는 스크립트 (`git/opt/*`, `scripts/windows/*.ps1` 등)
...
- **스크립트 활용**: 실제 키 생성과 config 추가는 `./git/opt/generate-ssh-key.sh`, `./git/opt/add-ssh-config.sh`, `./git/setup-github-account.sh` 또는 Windows 대응 스크립트를 사용해 수동 실수를 줄입니다.
...
- `./git/opt/setup-git-includeif.sh` 또는 PowerShell 스크립트를 사용하면 디렉터리 단위 Git 사용자 정보가 자동으로 주입됩니다.
